...
import com.hb.mapper.YongHuBiaoMapper;
...
public class YongHuSeviceImpl implements YongHuService {
	@Autowired
	private YongHuBiaoMapper yongHuBiaoMapper;
	
	@Override
	public String findObjectByName(String userName) {
		SysUser user = yongHuBiaoMapper.findUserByUserName(userName);
		user.getYongHuMing();
		user.getMiMa();
		return userName;
	}
...
//		sh=new SimpleHash("MD5",newPwd,salt, 1);
		//4.将新密码加密以后的结果更新到数据库
		int rows = yongHuBiaoMapper.updatePassword( user.getYongHuMing(), newPwd,cfgPwd);
		if (rows==0) {
	...
	public int insertUser(SysUser entity) {
		int rows = yongHuBiaoMapper.insertUser(entity);
		return rows;
	}

	@Override
	public int findByPhone(String phone) {
		int rows = yongHuBiaoMapper.findByPhone(phone);
		return rows;
	}
